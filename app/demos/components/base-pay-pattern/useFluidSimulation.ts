/**
 * 流体模拟 Hook - 基于 Navier-Stokes 方程的 WebGL 实现
 */
...
interface DoubleFBO {
  read: { texture: WebGLTexture; fbo: WebGLFramebuffer; width: number; height: number };
  write: { texture: WebGLTexture; fbo: WebGLFramebuffer; width: number; height: number };
  swap: () => void;
}
...
export class FluidSimulation {
  ...
  constructor(gl: WebGL2RenderingContext, config: FluidConfig = {}) {
    ...
    this.config = {
      ...
      densityDissipation: config.densityDissipation ?? 0.95,
      velocityDissipation: config.velocityDissipation ?? 0.9,
      pressureIterations: config.pressureIterations ?? 20,
      curl: config.curl ?? 30,
      splatRadius: config.splatRadius ?? 0.003,
    };
  ...
  private getGradientSubtractShader() {
    return `#version 300 es
    precision highp float;
    in vec2 vUv;
    in vec2 vL;
  ...
  private createProgram(vertexSource: string, fragmentSource: string): Program | null {
    const gl = this.gl;
  ...
  private blit(target: SingleFBO | { fbo: WebGLFramebuffer; width: number; height: number } | null) {
    const gl = this.gl;
    if (target) {
