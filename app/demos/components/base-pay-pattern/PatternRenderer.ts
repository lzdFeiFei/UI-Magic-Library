...
export class PatternRenderer {
  ...
  public config: Required<PatternConfig>;
  private program: WebGLProgram | null = null;
  private imageTexture: WebGLTexture | null = null;
  private patternTexture: WebGLTexture | null = null;
  private altPatternTexture: WebGLTexture | null = null;
  ...
  constructor(gl: WebGL2RenderingContext, config: PatternConfig = {}) {
    ...
    this.config = {
      ...
      patternColumns: config.patternColumns ?? 4,
      ...
      deformStrength: config.deformStrength ?? 0.05,
      darkMode: config.darkMode ?? false,
      fadeThreshold: config.fadeThreshold ?? 0.1,
      fadeWidth: config.fadeWidth ?? 0.05,
      altPatternOpacity: config.altPatternOpacity ?? 1.0,
      imageScale: config.imageScale ?? 1.0,
      enableFadeTransition: config.enableFadeTransition ?? true,
    };
  ...
  private createDefaultPattern() {
    ...
    for (let col = 0; col < columns; col++) {
      const density = col / (columns - 1);
      const dotRadius = 0.5 + density * 3.0;
  ...
  setPatternAtlas(image: HTMLImageElement) {
    const gl = this.gl;
  ...
  setAltPatternAtlas(image: HTMLImageElement) {
    const gl = this.gl;
  ...
  render(deformTexture: WebGLTexture, time: number) {
    ...
    if (!this.imageTexture || !this.program) return;
    ...
    gl.uniform1f(this.uniforms.uImageScale, this.config.imageScale);
    ...
    gl.uniform1f(this.uniforms.uUseOriginalSvgColors, 1.0);
